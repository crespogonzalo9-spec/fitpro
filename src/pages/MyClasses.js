...
import { CheckSquare, Calendar, Clock, MapPin, User, X } from 'lucide-react';
import { Button, Card, EmptyState, LoadingState, Badge, ConfirmDialog } from '../components/Common';
import { useAuth } from '../contexts/AuthContext';
import { useGym } from '../contexts/GymContext';
import { useToast } from '../contexts/ToastContext';
import { db } from '../firebase';
import { collection, query, where, onSnapshot, deleteDoc, doc, getDocs, updateDoc, increment } from 'firebase/firestore';
  ...
  if (!currentGym) {
    return (
      <EmptyState
        icon={CheckSquare}
        title="Sin gimnasio"
        description="Seleccioná un gimnasio para ver tus clases"
      />
    );
  }
              ...
              <Card key={cls.id} className="border-primary/30">
                ...
                <div className="mt-4 space-y-2 text-sm text-gray-400">
                  <div className="flex items-center gap-2">
                    <Calendar size={14} />
                    <span>{cls.schedule?.map(s => getDayName(s.day)).join(', ') || 'Sin horario'}</span>
                  </div>
                  {cls.schedule?.[0] && (
                    <div className="flex items-center gap-2">
                      <Clock size={14} />
